...
%
% imdst = REPEAT(imsrc, [r,c,t])
%	works for 3D image matrix
%
...
if length(siz) == 2
 ...
 y = x(:,[ones(1,siz(2)), 1:cols, cols + zeros(1,siz(2))]);
 y = y([ones(siz(1),1); (1:rows).'; rows + zeros(siz(1),1)],:);
else
 [rows,cols,layers] = size(x);
 
 y = x(:,:,[ones(1,siz(3)), 1:layers, layers+zeros(1,siz(3))]);
 y = y(:,[ones(1,siz(2)), 1:cols, cols + zeros(1,siz(2))],:);
 y = y([ones(siz(1),1); (1:rows).'; rows + zeros(siz(1),1)],:,:);
